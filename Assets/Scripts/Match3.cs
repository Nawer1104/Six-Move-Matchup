...
public class Match3 : MonoBehaviour
{
    ...
    [SerializeField] GameObject gemPrefabSpecial;
    [SerializeField] GemType[] gemTypes;
    [SerializeField] GemType gemTypeSpeical;
    ...
    private bool isSwapDone = true;

    private GameObject gemSpecial;
    ...
    private void Start()
    {
        ...
        inputReader.Fire += InputReader_Fire;
    }

    private void InputReader_Fire(object sender, System.EventArgs e)
    {
    ...
    IEnumerator FillEmptySpots()
    {
        for (var x = 0; x < width; x++)
        {
            for (var y = 0; y < height; y++)
            {
                ...
                {
                    CreateGem(gemPrefab, x, y);
                    //audioManager.PlayPop();
                    yield return new WaitForSeconds(0.1f); ;
                }
            }
        }
        gemSpecial.GetComponentInChildren<TextMeshProUGUI>().text = gemSpecial.GetComponent<Gem>().SetTextNumOfType().ToString();
        isSwapDone = true;
    }
    ...
    void InitializeGrid()
    {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    CreateGemSpeical(gemPrefabSpecial, x, y);
                }
                else
                {
                    CreateGem(gemPrefab, x, y);
    ...
    void CreateGem(GameObject gemPf, int x, int y)
    {
        GameObject gem = Instantiate(gemPf, grid.GetWorldPositionCenter(x, y), Quaternion.identity, transform);
        gem.GetComponent<Gem>().SetType(gemTypes[Random.Range(0, gemTypes.Length)]);
        var gridObject = new GridObject<Gem>(grid, x, y);
        gridObject.SetValue(gem.GetComponent<Gem>());
        grid.SetValue(x, y, gridObject);
    }

    void CreateGemSpeical(GameObject gemPf, int x, int y)
    {
        GameObject gem = Instantiate(gemPf, grid.GetWorldPositionCenter(x, y), Quaternion.identity, transform);
        gemSpecial = gem;
        gem.GetComponent<Gem>().SetType(gemTypeSpeical);
        var gridObject = new GridObject<Gem>(grid, x, y);
        gridObject.SetValue(gem.GetComponent<Gem>());
        grid.SetValue(x, y, gridObject);
        gemSpecial.GetComponentInChildren<TextMeshProUGUI>().text = gemTypeSpeical.num.ToString();
    }
    ...
    bool IsSpecialGem(Vector2Int gridPosition)
    {
        var gridObject = grid.GetValue(gridPosition.x, gridPosition.y);
        return gridObject.GetValue().special;
    }
    
    // Init Grid

    // Read player input and swap
